package rain.com.dribbble_client.ui.views.behaviors;
...
public class AvatarImageBehavior extends CoordinatorLayout.Behavior<ImageView> {
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, ImageView child, View dependency) {
        ...
        float expandedPercentageFactor = dependency.getY() / maxScrollDistance;//1~0

        if (expandedPercentageFactor < mChangeBehaviorPoint) {
            float heightFactor = (mChangeBehaviorPoint - expandedPercentageFactor) / mChangeBehaviorPoint;//0~1
            Log.d("chj", "heightfactor:" + heightFactor);
...
//            mStartXPosition = dependency.getWidth() - child.getWidth()/2;
            mStartXPosition = (int) (child.getX() + (child.getWidth() / 2));

        if (mFinalXPosition == 0)
//            mFinalXPosition = dependency.getWidth() / 2;
